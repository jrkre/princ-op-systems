...
int main(int argc, char *argv[]){
  ...
  //inode_t* cur_dir_inode = NULL; // unused variable? -pedantic build flag gives warning so i removed it
...
void my_dumpfs(myfs_t* myfs) {
  ...
  printf("groupdescriptor_info.inode_table: %p\n", (void*)myfs->groupdescriptor.groupdescriptor_info.inode_table); // FTFY - -pedantic build flag gives warning w/o cast to (void*)
  printf("groupdescriptor_info.block_data: %p\n", (void*)myfs->groupdescriptor.groupdescriptor_info.block_data); // same here
...
void my_creatdir(myfs_t* myfs, int cur_dir_inode_number, const char* new_dirname) {
  ...
  for (int byte = 0; byte < BLKSIZE; ++byte) {
      for (int bit = 0; bit < 8; ++bit) {
          int inode_number = byte * 8 + bit;
          if (inode_number <= 2) continue; // CONTINUE - KEYWORD OF THE GODS
          if (!(imap->data[byte] & (0x1 << bit))) {
              imap->data[byte] |= (0x1 << bit);
              new_inode_number = inode_number;
              break;
          }
      }
      if (new_inode_number != -1) break;
  }
  ...
  // Find a free data block in the block bitmap
  void* bmap_ptr = malloc(BLKSIZE);
  memcpy(bmap_ptr, &myfs->bmap, BLKSIZE);
  block_t* bmap = (block_t*)bmap_ptr;
  int new_block_number = -1;
  int num_data_blocks = myfs->super.superblock_info.blocks;
  for (int i = 0; i < num_data_blocks; ++i) {
      if (!(bmap->data[i / 8] & (0x1 << (i % 8)))) {
          bmap->data[i / 8] |= (0x1 << (i % 8)); // set the bit
          // bmap->data[i] = 1; // this sets the whole byte to 1
          new_block_number = i;
          break;
      }
  }
  ...
  // modify inode table (parent and new dir)
  void* inodetable_ptr = malloc(BLKSIZE);
  inode_t* inodetable_disk = myfs->groupdescriptor.groupdescriptor_info.inode_table;
  memcpy(inodetable_ptr, inodetable_disk, BLKSIZE);
  inode_t* inodetable = (inode_t*)inodetable_ptr;

  // parent dir: increase size for new entry
  inode_t* parent_inode = &inodetable[cur_dir_inode_number];
  int parent_num_direntries = parent_inode->size / sizeof(dirent_t);
  ...
  new_inode->blocks = 1;
  for (int i = 1; i < 15; ++i) new_inode->data[i] = NULL;
  block_t* block_data = myfs->groupdescriptor.groupdescriptor_info.block_data;
  new_inode->data[0] = &block_data[new_block_number];

  memcpy(inodetable_disk, inodetable_ptr, BLKSIZE);
  free(inodetable_ptr);

  // modify parent directory's data block with new entry
  dirent_t* parent_direntries = (dirent_t*)(parent_inode->data[0]);
  dirent_t* new_entry = &parent_direntries[parent_num_direntries];
  ...
  // init new directory's data block w/ . and ..
  dirent_t* newdir_direntries = (dirent_t*)(new_inode->data[0]);

  newdir_direntries[0].inode = new_inode_number;
  newdir_direntries[0].file_type = 2;
  newdir_direntries[0].name_len = 1;
  strcpy(newdir_direntries[0].name, ".");
  
  newdir_direntries[1].inode = cur_dir_inode_number;
  newdir_direntries[1].file_type = 2;
  newdir_direntries[1].name_len = 2;
  strcpy(newdir_direntries[1].name, "..");
}
